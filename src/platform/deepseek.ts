 ...
 */
export class DeepseekLLM extends LLMRunner.BaseLLM {
  private cgpt: OpenAI;
  private model: DeepseekModel;

  public constructor(api_key: string, model: DeepseekModel, chat_session_manager?: GenericLLM.ChatSessionManager<LLMRunner.ChatSession, LLMRunner.Message>) {
    super(chat_session_manager ?? new LLMRunner.SessionManager());
    this.cgpt = new OpenAI({ apiKey: api_key, baseURL: 'https://api.deepseek.com' });
    this.model = model;
  }
